...
{
    public partial class ComandaReporte : Form
    {
        ...
        private void btnGenerar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var lista = _ComandaRepo.ComandaReporte(DateTime.Parse(dtpDesde.Value.ToString("yyyy-MM-dd")), DateTime.Parse(dtpFechaHasta.Value.ToString("yyyy-MM-dd")));
                    if (lista.Count > 0)
                    {
            ...
            {
                _Log.Error($"ComandaReporte exception: {ex}");
        ...
        private string GenerarMensaje(List<ComandaReport> lista)
        {
            string retorno = string.Empty;
            ...
            try
            {
                ...
                {
                    retorno += String.Format("Menu de Venta: {0}{1}{0}. Fecha:{0}{2}{0}. Operador de Venta: {0}{3}{0}.", Environment.NewLine, item.Menu, item.Fedate, item.Nombre);
                }
            }
            catch (Exception ex)
            {
